...
class RenderToCSVTests {
  ...
  "renders csv header"() {
    ...
    expect(header).to.equal("Start, End, Count, Min Age, Median Age, Max Age, Min Activity, Median Activity, Max Activity, Min File Count, Median File Count, Max File Count, Min Change Size, Median Change Size, Max Change Size, Min Age ID, Max Age ID")
  }
  ...
  "renders csv row"() {
    ...
    const row = csv.split("\n")[1]
    // tslint:disable-next-line:max-line-length
    expect(row).to.equal("2018-09-09, 2018-09-15, 3, 2.00, 4.00, 6.00, 1.00, 2.00, 3.00, 1, 5, 10, 20, 100, 200, https://example.com/pr/1, https://example.com/pr/3")
  }
